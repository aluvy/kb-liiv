...
function sendOP0(){
	...
	if($("#npModelAmt").val() == "0" || $("#npModelAmt").val() == ""){ //번호이동 단말기 할부금 처리
		//완납(1), 분납지속(LMS미청구)(2), 분납지속(LMS청구)(3)
		//타사 단말할부금 = 0이면 "1"(완납) 만 가능 
		inNpDto.npHndsetInstaDuratDivCd = "1"; //번호이동단말기분납지속구분코드
		inNpDto.npHndsetInstaLmsTlphNo = ""; //번호이동단말기분납문자명세서전화번호
	}
	else{
		//완납(1), 분납지속(LMS미청구)(2), 분납지속(LMS청구)(3)
		//타사 단말할부금 > 0 이면 3으로  ==> SKT, KT, LGU일땐 3가능, 나머지통신사 2로
		if($("#mcoCd").val() == "SKT" || $("#mcoCd").val() == "KTF" || $("#mcoCd").val() == "LGT"){
			inNpDto.npHndsetInstaDuratDivCd = "3"; //번호이동단말기분납지속구분코드
			inNpDto.npHndsetInstaLmsTlphNo = $("#mtelNo").val(); //번호이동단말기분납문자명세서전화번호
		}
		else{
			inNpDto.npHndsetInstaDuratDivCd = "2"; //번호이동단말기분납지속구분코드
			inNpDto.npHndsetInstaLmsTlphNo = $("#mtelNo").val(); //번호이동단말기분납문자명세서전화번호	
		}
	}
	
	inNpDto.npFeePayMethCd = "Y"; //번호이동수수료 수납방법코드
	inNpDto.npBillMethCd = "Y"; //타사미청구금액 청구방법코드
	inNpDto.rfundNpBankCd = ""; //번호이동환불은행코드
	inNpDto.rfundBankBnkacnNo = ""; //번호이동환불계좌번호
	inNpDto.npTotRmnyAmt = "0"; //번호이동총수납금액
	inNpDto.npCashRmnyAmt = "0"; //번호이동현금수납액
	inNpDto.npCcardRmnyAmt = "0"; //번호이동카드수납액
	inNpDto.npRmnyMethCd = "01"; //번호이동수납방법코드
	...
	osstNpOpenPrc.OsstOpenPrcInVO = OsstOpenPrcInVO;
	data.osstNpOpenPrc = osstNpOpenPrc;
		...
		success: function(response) {
			...
			try{
				if(response !== null && response.data !== null){
					var res = response.data.osstNpOpenPrcResponse.return;
					...
					if(res.commHeader.responseType == "N"){
						...
						if(vPrgrStatCd == "OP2"){
							...
							if(gStatCount < 10){
								...
								hideOpenBar();
								popalarm2("["+vRsltCd+"]"+vRsltMsg+"<br/>개통 완료 상태를 확인하지 못했습니다.<br/>5분 후 재 로그인하여 다시 한번 개통완료 여부를 확인해 주세요.", "info", false);
								$("#SB816_RE").val("Y");
								return;
							}
						}
